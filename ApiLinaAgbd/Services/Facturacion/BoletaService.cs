...
namespace ApiLinaAgbd.Services.Facturacion
{
	...
	{
		private readonly BoletaUblBuilder _builder;
		...
		public Task<FacturacionEnvioResultado> Enviar(BoletaRequestDto request)
		{
			...
			var fileName = $"{_settings.Emisor.Ruc}-{BoletaUblBuilder.TipoDocumentoBoleta}-{request.Serie}-{request.Correlativo}";
