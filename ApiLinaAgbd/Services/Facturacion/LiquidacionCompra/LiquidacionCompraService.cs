...
{
	public class LiquidacionCompraService : ILiquidacionCompraService
	{
		private const string SerieLiquidacion = "LC01";
		private readonly ILiquidacionCompraRepository _repository;
		...
		public LiquidacionCompraService(
			ILiquidacionCompraRepository repository,
			...
			const string sql = """
				...
				WHERE NOT EXISTS (
					...
					WHERE v.CompraId = c.id
					  ...
					  AND ISNULL(v.SunatStatus, 'NO_ENVIADO') IN ('NO_ENVIADO', 'PENDIENTE', 'ACEPTADO', 'EXCEPCION')
				)
